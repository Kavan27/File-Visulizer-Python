...
class PopulationTree(AbstractTree):
    ...
    def get_separator(self: PopulationTree) -> None:
...
def _load_data() -> List[PopulationTree]:
    ...
    for x in regions:
        temp = []
        for y in regions[x]:
            if y in country_populations:
                temp.append(PopulationTree(False, y, None,
                                           country_populations[y]))
        main.append(PopulationTree(False, x, temp))
    return main
...
def _get_population_data() -> Dict[str, int]:
    ...
    for x in population_data:
        ...
        for key, value in x.items():
            ...
            if key == "value" and isinstance(value, int):
                temp.append(value)
        if len(temp) == 2:
            countries[temp[0]] = temp[1]
    return countries
...
def _get_region_data() -> Dict[str, List[str]]:
    ...
    for x in country_data:
        if x['region']['value'] != 'Aggregates':
            if x['region']['value'] not in regions:
                regions[x['region']['value']] = []
            regions[x['region']['value']].append(x['name'])
    return regions
